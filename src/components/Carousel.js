...
const Carousel = () => {
  ...
  return (
    <div className='max-w-[1240px] mx-auto px-4 py-16 relative flex justify-center items-center' id='view'>
      <BsArrowLeftCircleFill onClick={prevSlide} className='absolute top-[50%] text-3xl text-white cursor-pointer left-8' />
      <BsArrowRightCircleFill onClick={nextSlide} className='absolute top-[50%] text-3xl text-white cursor-pointer right-8' />
      {sliderData.map((item, index) => (
        <div key={index} className={`transition-opacity duration-500 ${index === slide ? 'opacity-100' : 'opacity-0'}`}>
          {index === slide && (
            <img className='w-full rounded-md transition-opacity duration-500' src={item.url} alt='carousel' />
          )}
        </div>
      ))}
    </div>
